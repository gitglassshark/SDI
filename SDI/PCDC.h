...
{
private:
	std::map<std::wstring, clock_t> ClockM;
	std::map<std::wstring, size_t> CountM;
	std::wstring Cname;
	clock_t Cclock = 0;
	size_t Ccount = 0;
public:
	...
	bool init_clock_item(const std::wstring& name, clock_t in = 0)
	{
		ClockM[name] = in;
		return true;
	}
	bool init_count_item(const std::wstring &name, size_t in = 0)
	{
		CountM[name] = in;
		return true;
	}
	...
	ClockCount& operator[](const wchar_t * const name)
	{
		Cname = name;
		Cclock = 0;
		return *this;
	}
	ClockCount& operator[](const std::wstring& name)
	{
		Cname = name;
		Cclock = 0;
		return *this;
	}
	bool start_clock_count()
	{
		Cclock = clock();
		return true;
	}
	bool end_clock_count()
	{
		Cclock = clock() - Cclock;
		ClockM[Cname] += Cclock;
		Cclock = 0;
	...
	std::map<std::wstring, size_t>& count_list()
	{
		return CountM;
	}
	std::map<std::wstring, clock_t>& time_list()
	{
		return ClockM;
	}
	size_t count_add(size_t c)
	{
		return CountM[Cname] += c;
	}
	size_t count_add()
	{
		return ++CountM[Cname];
...
public:
	...
	inline bool storesms(bool isstorecomand = true)
	{
		size_t maxline = getmaxline();
		size_t totalmaxline = maxline * maxrecode;
		auto start = clock();
		++log_clock_count[L"记录调用"];
		if (!isstorecomand)
			return true;
		if (isurerecode)
		{
			if (!sms.IsEmpty()) {
				if (mvlogs.size() >= totalmaxline)
				{
					...
					++log_clock_count[L"记录调整"];
				}
				mvlogs.push_back(sms);
				sms.Empty();
				++log_clock_count[L"空记录2"];
			}
			++log_clock_count[L"空记录3"];
		}
		auto end = clock();
		log_clock_count[L"记录总耗时"].count_add(end - start);
		return false;
	}
	...
	template<typename S> inline void imresizeout(S&& cs) //requires Xstring<S&>
	{
		auto start = clock();
		storesms(false);
		++log_clock_count[L"输出总调用"];
		if (cs.IsEmpty()) {
			++log_clock_count[L"空输出调用"];
			return;
		}
		m_pwnd->GetClientRect(&mrect);
		msize = this->GetOutputTextExtent(cs);
		...
		if (strlen <= linelen) {
			if (p.y >= mrect.bottom - mrect.top - initalpos) {
				++log_clock_count[L"满屏"];
				flushscreen();
			}
			++log_clock_count[L"行内输出"];
			//need recalc position***
			if (p.x == mrect.left + initalpos) {
				storesms();
			}
			auto tstart = clock();
			this->TextOutW(p.x, p.y, cs);
			++log_clock_count[L"API调用"];
			auto tend = clock();
			log_clock_count[L"API总耗时"].count_add(tend-tstart);
			recode(cs);
			p.x += msize.cx;

			//need check position***
			if (p.x >= (mrect.right - initalpos * 2)) {
				++log_clock_count[L"换行"];
				storesms();
				rinitx();
				//p.x = mrect.left + initalpos;
				p.y += step;
		...
		auto end = clock();
		log_clock_count[L"输出总耗时"].count_add(end-start);
	}
